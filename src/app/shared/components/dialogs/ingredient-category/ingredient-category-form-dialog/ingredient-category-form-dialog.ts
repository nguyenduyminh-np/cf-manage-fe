...
export class IngredientCategoryFormDialog implements OnInit {
  ...
  ngOnInit(): void {
    this.http
      .get<ApiListResponse<IngredientCategoryOption>>(`${BASE}/ingredient-category/options`)
      .toPromise()
      .then((r) => {
        const options = r?.data ?? [];
        // Filter out itself if editing
        const filtered = this.isEdit ? options.filter((o) => o.id !== this.src?.id) : options;
  ...
  protected submit(): void {
    ...
    if (this.isEdit) {
      const payload: IngredientCategoryUpdateRequest = {
        ...
        parentCategoryId:
          this.formData.parentCategoryId > 0 ? this.formData.parentCategoryId : undefined,
